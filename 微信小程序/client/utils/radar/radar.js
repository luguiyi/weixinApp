let width = 250; // canvas的宽
let height = 250; // canvas的高

let angleNum = 6; // 角数，即多少个数据项 【3，10】
let angleAvg = 360 / angleNum; // 分角度平均值
let angleOffset = 0; // 角度偏移量
let layerNum = 100; // 层数，即环层数量
let layerWidth = 0; // 层宽
let bgLineColor = '#2c313f'; // 背景线条颜色
...
let radar = {
    init: function(data) {
      dataArr = data;
      wordArr = ['外交(' + data[0] + ')', '人事(' + data[1] + ')', '智谋(' + data[2] + ')', '军事(' + data[3] + ')', '勇武(' + data[4] + ')', '政务(' + data[5] + ')'];
        centerPoint = [rpx(168), rpx(125)];
        layerWidth = rpx(1);
        bgLineWidth = rpx(0.1);
        fontSize = rpx(20);
        circleWidth = rpx(1);
